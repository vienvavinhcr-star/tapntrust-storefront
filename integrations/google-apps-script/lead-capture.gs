...
/**
 * One-time setup:
 * 1. Create/open the Google Sheet that should receive Tapntrust leads.
 * 2. Extensions -> Apps Script.
 * 3. Replace Code.gs with this file.
 * 4. Run setupTapntrustLeadSheet() once and approve permissions.
  ...
  try {
    ...
    try {
      ...
      const values = [
        ...
        visitorId,
        addToCartAt ? 'YES' : '',
        addToCartAt ? formatDate_(addToCartAt, 'dd/MM/yyyy HH:mm:ss') : '',
        checkoutAt ? 'YES' : '',
        checkoutAt ? formatDate_(checkoutAt, 'dd/MM/yyyy HH:mm:ss') : '',
        eventName,
        ...
        clean_(data.page) || clean_(row[11]),
        clean_(data.referrer) || clean_(row[12]),
...
function findLeadRow_(sheet, visitorId, email) {
  ...
  for (let index = values.length - 1; index >= 0; index -= 1) {
    const rowEmail = clean_(values[index][0]).toLowerCase();
    const rowVisitorId = clean_(values[index][3]);
    if (visitorId && rowVisitorId === visitorId) return index + 2;
    if (normalEmail && rowEmail === normalEmail) return index + 2;
  }
  return 0;
}
