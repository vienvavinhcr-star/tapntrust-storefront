...
assert(/CHECKOUT_EXTRA_NOTICE_DELAY_MS\s*=\s*10000/.test(offer), "Checkout notice must open after 10 seconds.");
const checkoutNotice = offer.match(/function buildCheckoutExtraNotice[\s\S]*?\n}\nfunction buildCartOffer/)?.[0] || "";
assert(/Enjoy your extra[\s\S]*\$\{discountPercent\}% off[\s\S]*at checkout/.test(checkoutNotice), "Timed notice must present the checkout discount clearly.");
assert(/tapntrust-insights-popup-mascot\.png/.test(checkoutNotice), "Timed notice must use the approved Tapntrust mascot.");
assert(!/data-welcome-email|type="email"|<form/.test(checkoutNotice), "Timed notice must not ask for an email or render a form.");
...
assert(/claimed\(\) && hasPrimary\(latestCartState\) && !cartDiscountApplied\(\)/.test(offer), "A pre-cart claim must apply when a primary product reaches the cart.");
