...
describe("storefront Insights selling-plan offer", () => {
  ...
  it("quotes a verified business on the intro selling plan", async () => {
    const response = await requestOffer({
      action: "quote",
    ...
    expect(payload.sellingPlanId).toBe(String(env.SHOPIFY_INSIGHTS_INTRO_SELLING_PLAN_ID));
  ...
  it("issues and reuses an intro reservation without creating a Shopify discount", async () => {
    const body = {
      action: "issue",
      setupId: unique("setup"),
      businessName: "New Test Business",
    ...
    const firstPayload = await first.json<{ offerId: string; sellingPlanId: string; discountCode?: string }>();
    expect(firstPayload.offerId).toMatch(/^offer_/);
    expect(firstPayload.sellingPlanId).toBe(String(env.SHOPIFY_INSIGHTS_INTRO_SELLING_PLAN_ID));
    expect(firstPayload.discountCode).toBeUndefined();
    ...
    const secondPayload = await second.json<{ offerId: string; sellingPlanId: string }>();
    expect(secondPayload.offerId).toBe(firstPayload.offerId);
    expect(secondPayload.sellingPlanId).toBe(firstPayload.sellingPlanId);
  });

  it("returns the standard selling plan for a verified business that already consumed its intro", async () => {
