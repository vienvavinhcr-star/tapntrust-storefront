...
function usageTarget(request: Request, url: URL, response: Response): UsageTarget | null {
  ...
  if (request.method !== "GET" || (response.status !== 200 && response.status !== 429)) return null;
...
async function trackCustomerUsage(
  ...
  const actor = await resolveUsageActor(env.DB, rawSessionToken, target.requestedLocationId, createdAt);
  if (!actor) return;
  ...
  const outcome = target.responseForTracking ? await googleOutcome(target.responseForTracking) : "unknown";
  ...
  await recordUsage(env.DB, actor, target.eventType, outcome, providerCalled, createdAt);
}
