...
<body>
  ...
  <script>
    ...
    const loginPanel=document.querySelector('#login-panel'),dashboard=document.querySelector('#dashboard'),account=document.querySelector('#account'),message=document.querySelector('#login-message'),form=document.querySelector('#login-form'),button=document.querySelector('#send-link'),accountEmail=document.querySelector('#account-email'),monthCount=document.querySelector('#month-count'),businesses=document.querySelector('#businesses');
    let summaryRequest=null,refreshTimer=null,authenticated=false;
    ...
    function showLogin(){authenticated=false;stopPolling();loginPanel.hidden=false;dashboard.hidden=true;account.hidden=true;accountEmail.textContent='';monthCount.textContent='0';businesses.replaceChildren();message.hidden=true}
    ...
    function renderDashboard(data){accountEmail.textContent=data.email;monthCount.textContent=data.monthTapCount;businesses.innerHTML=data.businesses.map(businessMarkup).join('')||'<section class="panel"><h2>No business access yet</h2><p class="muted">Contact Tapntrust Support so we can connect this account to your business.</p></section>';loginPanel.hidden=true;dashboard.hidden=false;account.hidden=false;authenticated=true;startPolling()}
    function load({showInitialError=false}={}){if(summaryRequest)return summaryRequest;summaryRequest=(async()=>{try{const response=await api('/api/customer/summary');if(response.status===401){showLogin();return}if(!response.ok)throw new Error('Could not load your insights.');renderDashboard(await response.json())}catch(error){if(showInitialError&&!authenticated){message.className='error';message.textContent='Could not load your dashboard. Please try again.';message.hidden=false}}finally{summaryRequest=null}})();return summaryRequest}
