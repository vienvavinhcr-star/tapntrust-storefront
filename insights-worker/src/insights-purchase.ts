...
function planIdForEligibility(env: InsightsPurchaseEnv, eligibility: EligibilityResolution): string {
  return eligibility.eligibleIntro
    ? clean(env.SHOPIFY_INSIGHTS_INTRO_SELLING_PLAN_ID, 240)
    : clean(env.SHOPIFY_INSIGHTS_STANDARD_SELLING_PLAN_ID, 240);
}
...
function offerPayload(env: InsightsPurchaseEnv, eligibility: EligibilityResolution, extra: Record<string, unknown> = {}) {
  return {
    ...
    sellingPlanId: planIdForEligibility(env, eligibility),
    ...extra
  };
}

function introPlanConfigured(env: InsightsPurchaseEnv): boolean {
  const intro = clean(env.SHOPIFY_INSIGHTS_INTRO_SELLING_PLAN_ID, 240);
  const standard = clean(env.SHOPIFY_INSIGHTS_STANDARD_SELLING_PLAN_ID, 240);
  return Boolean(intro && standard && intro !== standard);
}
...
export async function handleInsightsPurchaseRequest(
  ...
  if (!introPlanConfigured(env)) {
    console.error(JSON.stringify({ message: "insights intro selling plan configuration is not ready" }));
