...
<body>
  <main class="shell">
    ...
    <div id="dashboard" hidden>
      ...
      <section class="panel"><h2>Recent provisioning</h2><div style="overflow:auto"><table class="recent"><thead><tr><th>Created</th><th>Order / setup</th><th>Business</th><th>Cards</th><th>Programming</th></tr></thead><tbody id="batches"></tbody></table></div></section>
  ...
  <script>
    ...
    const copyText=async value=>{const text=String(value||'');if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(text);return}const area=document.createElement('textarea');area.value=text;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.append(area);area.select();document.execCommand('copy');area.remove()};
    ...
    document.querySelector('#cards').addEventListener('click',async event=>{const button=event.target.closest('[data-copy-card-url]');if(!button)return;const label=button.textContent;try{await copyText(button.dataset.copyCardUrl);button.textContent='Copied';setTimeout(()=>{button.textContent=label},1200)}catch{alert('Could not copy the programming URL.')}});
  ...
  <script>
    (()=>{
      ...
      async function loadOperations(){const [optionData,batchData]=await Promise.all([opsRequest('/api/admin/provisioning/options'),opsRequest('/api/admin/provisioning/batches')]);options=optionData.businesses;fillSelectors();document.querySelector('#batches').innerHTML=batchData.batches.map(batch=>'<tr><td>'+opsEsc(new Date(batch.createdAt).toLocaleString())+'</td><td>'+opsEsc(batch.externalOrderReference)+'<br><span class="token">'+opsEsc(batch.externalSetupReference)+'</span></td><td>'+opsEsc(batch.businessName)+'</td><td>'+batch.physicalCardCount+'</td><td><button class="button secondary compact" type="button" data-view-manifest="'+opsEsc(batch.id)+'">View URLs</button></td></tr>').join('')||'<tr><td colspan="5" class="muted">No cards provisioned yet.</td></tr>'}
      async function refresh(){await Promise.all([loadOperations(),typeof load==='function'?load():Promise.resolve()])}
      function renderManifest(result){const manifest=result.manifest,box=document.querySelector('#manifest'),urls=manifest.cards.map(card=>card.programmingUrl);box.innerHTML='<strong>'+opsEsc(manifest.businessName)+'</strong><div class="muted">'+opsEsc(manifest.externalOrderReference)+' · '+manifest.physicalCardCount+' physical cards'+(result.replayed?' · Existing manifest returned':'')+'</div><ol>'+manifest.cards.map(card=>'<li><div class="programming-url"><span><strong>Card '+card.ordinal+'</strong><br><span class="token">'+opsEsc(card.programmingUrl)+'</span></span><button class="button secondary compact" type="button" data-copy-manifest-url="'+opsEsc(card.programmingUrl)+'">Copy URL</button></div></li>').join('')+'</ol><div class="manifest-actions"><button class="button secondary compact" type="button" data-copy-all-urls="'+opsEsc(urls.join(String.fromCharCode(10)))+'">Copy all URLs</button></div>';box.hidden=false;box.scrollIntoView({behavior:'smooth',block:'nearest'})}
      ...
      document.querySelector('#batches').addEventListener('click',async event=>{const button=event.target.closest('[data-view-manifest]');if(!button)return;button.disabled=true;try{const data=await opsRequest('/api/admin/provisioning/batches/'+encodeURIComponent(button.dataset.viewManifest));renderManifest({manifest:data.manifest,replayed:true})}catch(error){show('#provision-message',error.message,true)}finally{button.disabled=false}});
      document.querySelector('#manifest').addEventListener('click',async event=>{const button=event.target.closest('[data-copy-manifest-url],[data-copy-all-urls]');if(!button)return;const value=button.dataset.copyManifestUrl||button.dataset.copyAllUrls||'',label=button.textContent;try{await copyText(value);button.textContent='Copied';setTimeout(()=>{button.textContent=label},1200)}catch{show('#provision-message','Could not copy the programming URL.',true)}});
      provisionForm.addEventListener('submit',async event=>{event.preventDefault();const button=provisionForm.querySelector('button[type="submit"]');button.disabled=true;try{const data=Object.fromEntries(new FormData(provisionForm));data.physicalCardCount=Number(data.physicalCardCount);const result=await opsRequest('/api/admin/provisioning/batches',{method:'POST',body:JSON.stringify(data)});renderManifest(result);if(provisionForm.elements.activateNow.checked){try{await opsRequest('/api/admin/insights/activations',{method:'POST',body:JSON.stringify({email:provisionForm.elements.customerEmail.value,businessId:result.manifest.businessId,locationId:result.manifest.locationId})});show('#provision-message','Cards provisioned and Insights activated.')}catch(activationError){show('#provision-message','Cards were provisioned, but Insights activation failed: '+activationError.message,true)}}else show('#provision-message','Cards provisioned. Programming URLs are ready to copy below.');await refresh()}catch(error){show('#provision-message',error.message,true)}finally{button.disabled=false}});
