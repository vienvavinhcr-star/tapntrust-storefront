...
export function createCartUi({ cartActions, formatMoney, updatePackagesFromCatalog, onEditBusiness }) {
  ...
  function createCartLine(line, currency) {
    ...
    if (line.variantTitle && line.kind !== "insights") {
    ...
    if (line.kind === "stand" || line.kind === "extra" || line.kind === "insights") {
      ...
      if (line.kind === "stand") {
        ...
        noteTitle.textContent = "Your private Insights dashboard";
        noteText.textContent = "Secure sign-in link sent by email after purchase. Renews at A$6.99/month · Cancel anytime.";
      } else {
