...
- Tap redirects, customer auth or owner insights: read `docs/INSIGHTS.md`, then inspect `insights-worker/` only. Do not change storefront fulfilment URLs unless the task explicitly activates tracked-card fulfilment.
...
24. Customer Insights APIs must derive business access from the authenticated server-side session and `customer_business_access`. Never trust a client-supplied `business_id`, and never expose `ADMIN_API_TOKEN` to customer authentication or customer pages.
25. **Every physical Tapntrust NFC card uses `https://go.tapntrust.com/t/{publicToken}`.** There is no direct-Google card variant. Provision exactly one immutable server-generated token per physical card, record privacy-minimised taps from day one, and keep redirect/tap recording independent of Insights entitlement.
26. Universal card provisioning and customer Insights activation are separate concepts. A non-Insights order creates business/location/card/provisioning records only; it must not create `customer_users`, `customer_business_access`, send a magic link, or store purchaser email in D1. Later activation must reuse the existing records, tokens and tap history.
27. The production iPhone/Gmail magic-link flow from PR #21 is an invariant: non-consuming GET confirmation, explicit form POST, 15-minute single-use link, 30-day secure session and ZeptoMail transport. Do not refactor or weaken it during unrelated work.
...
- `insights-worker/src/customer-auth.ts`, `customer-repository.ts`, `customer-page.ts` — passwordless customer sessions, server-enforced tenant access and the separate customer dashboard.
